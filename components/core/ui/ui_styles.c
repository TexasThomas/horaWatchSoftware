...
// Timer - Cyan
#define COLOR_CYAN          lv_color_hex(0x00E5E5)

// Category colors
#define COLOR_YELLOW        lv_color_hex(0xE5A800)   // Reading
#define COLOR_RED           lv_color_hex(0xFF3B30)   // Business
#define COLOR_GREEN         lv_color_hex(0x30D158)   // Studying

// Button/Picker background
#define COLOR_BTN_BG        lv_color_hex(0x2A2A2A)
#define COLOR_PICKER_BG     lv_color_hex(0x1C1C1E)
...
static lv_style_t style_picker_frame;
static lv_style_t style_picker_item;
...
void ui_styles_init(void)
{
    ...
    // ===== Today Text (color set dynamically) =====
    ...
    // ===== Picker Frame (cyan border like in image) =====
    lv_style_init(&style_picker_frame);
    lv_style_set_bg_color(&style_picker_frame, COLOR_PICKER_BG);
    lv_style_set_bg_opa(&style_picker_frame, LV_OPA_COVER);
    lv_style_set_border_color(&style_picker_frame, COLOR_CYAN);
    lv_style_set_border_width(&style_picker_frame, 2);
    lv_style_set_radius(&style_picker_frame, 12);
    lv_style_set_pad_all(&style_picker_frame, 8);
    
    // ===== Picker Item =====
    lv_style_init(&style_picker_item);
    lv_style_set_bg_opa(&style_picker_item, LV_OPA_TRANSP);
    lv_style_set_border_width(&style_picker_item, 0);
    lv_style_set_pad_ver(&style_picker_item, 10);
    lv_style_set_pad_hor(&style_picker_item, 20);
}
...
lv_style_t* ui_style_picker_frame(void)    { return &style_picker_frame; }
lv_style_t* ui_style_picker_item(void)     { return &style_picker_item; }